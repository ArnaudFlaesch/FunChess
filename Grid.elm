...
renderTile model y x =
    img [onClick (getActionMsg x y model),
         style
            [("background-image", "url(images/"++(getTileColorToDisplay x y)++".png)")
            ,("min-height", "64px")
            ,("min-width", "64px")
            ,("border", "solid 0.2em")
            ,("border-color", if (canPawnMoveHere model x y) then "red" else "#340000")
            ],
...
getImageToDisplay x y model =
  case searchIfPieceIsOnTile model.pawnsList x y of
    Just value -> "images/pawns/"++value.pawnColor++"/"++value.pawnType++".png"
    Nothing -> "images/"++(getTileColorToDisplay x y)++".png"